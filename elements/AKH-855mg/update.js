...
function init(){
    ...
    let uppy = new Uppy.Uppy(uppyGeneralSettings)
...
uppy.use(Uppy.Instagram, {
   ...
   */
	...
	instance.data.fileDone = function (err, url){
      ...
      } else if(instance.data.fileCount === instance.data.fileList.length ){
    ...
    instance.data.publishFileStates = (file) => {
        ...
        instance.publishState('number_of_files', files.length );
        instance.publishState('all_file_names', files.map(x=>x.name));
    }
